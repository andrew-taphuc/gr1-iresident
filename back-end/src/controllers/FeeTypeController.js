...
export const getAllFeeTypes = async (req, res) => {
  try {
    ...
    const feeTypes = await feeTypeServices.getAllFeeTypes(apartmentId);
    res.status(200).json({ error: false, feeTypes });
  } catch (error) {
    res
      .status(500)
      .json({ error: true, message: "Error retrieving fee types", error });
  }
};
...
export const getFeeTypeById = async (req, res) => {
  try {
    ...
    if (!feeType)
      return res
        .status(404)
        .json({ error: true, message: "FeeType not found" });
    res.status(200).json({ error: false, feeType });
  } catch (error) {
    res
      .status(500)
      .json({ error: true, message: "Error retrieving fee type", error });
  }
};
...
export const createFeeType = async (req, res) => {
  try {
    ...
    if (!ApartmentID || !FeeTypeName || !Category || !Scope) {
      return res
        .status(400)
        .json({ error: true, message: "Missing required fields" });
    }
    const newFeeType = await feeTypeServices.createFeeType({
      ApartmentID,
      FeeTypeName,
      Description,
      Category,
    ...
    res
      .status(500)
      .json({ error: true, message: "Error creating fee type", error });
  }
};
...
export const updateFeeType = async (req, res) => {
  try {
    const updated = await feeTypeServices.updateFeeType(
      req.params.id,
      req.body
    );
    if (!updated)
    ...
    res
      .status(500)
      .json({ error: true, message: "Error updating fee type", error });
  }
};
...
export const deleteFeeType = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ error: false, message: "FeeType deleted successfully" });
  } catch (error) {
    res
      .status(500)
      .json({ error: true, message: "Error deleting fee type", error });
  }
};
