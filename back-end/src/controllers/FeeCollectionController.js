...
export const getFeeCollectionById = async (req, res) => {
  try {
    const feeCollection = await feeCollectionServices.getFeeCollectionById(
      req.params.id
    );
    if (!feeCollection)
      return res
        .status(404)
        .json({ error: true, message: "FeeCollection not found" });
    res.status(200).json({ error: false, feeCollection });
  } catch (error) {
    res
      .status(500)
      .json({ error: true, message: "Error retrieving fee collection", error });
  }
};
...
export const createFeeCollection = async (req, res) => {
  try {
    ...
    const newFeeCollection = await feeCollectionServices.createFeeCollection({
      FeeTypeID,
      CollectionName,
      StartDate,
      EndDate,
    ...
    res
      .status(500)
      .json({ error: true, message: "Error creating fee collection", error });
  }
};
...
export const updateFeeCollection = async (req, res) => {
  try {
    const updated = await feeCollectionServices.updateFeeCollection(
      req.params.id,
      req.body
    );
    if (!updated)
    ...
    res
      .status(500)
      .json({ error: true, message: "Error updating fee collection", error });
  }
};
...
export const deleteFeeCollection = async (req, res) => {
  try {
    const deleted = await feeCollectionServices.deleteFeeCollection(
      req.params.id
    );
    if (!deleted)
      return res
    ...
    res
      .status(200)
      .json({ error: false, message: "FeeCollection deleted successfully" });
  } catch (error) {
    res
      .status(500)
      .json({ error: true, message: "Error deleting fee collection", error });
  }
};
